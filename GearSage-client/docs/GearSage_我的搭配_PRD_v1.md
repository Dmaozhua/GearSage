...
本 PRD 中关于「公开 / 私密」「`isPublic`」「公开搭配必须只包含公开装备」的口径，已由 `docs/GearSage_我的装备_我的搭配_展示控制需求变更_v1.md` 覆盖。

当前执行口径：

1. 我的搭配的对外展示语义改为 `showOnProfile`：是否展示到主页。
2. 接口出参新增计算字段 `profileDisplayStatus`：`not_displayed / showing / invalid`。
3. 接口出参新增 `profileBlockedReasons`，用于解释展示异常。
4. 前端文案使用「展示到主页 / 取消主页展示 / 未展示 / 主页展示中 / 展示异常」，不再使用「公开 / 私密 / 公开受限」作为搭配管理文案。
5. 他人主页只展示 `showOnProfile=true` 且 `profileDisplayStatus=showing` 的代表搭配，最多 3 套。
6. 搭配引用装备被删除、状态不是 `owned`、装备库记录不可用或竿轮兼容失效时，不阻止用户管理操作，但该搭配进入展示异常并从他人主页隐藏。
7. 求推荐发布页选择我的搭配不受主页展示状态影响。

除以上展示控制口径外，搭配创建、装备归属校验、数量限制、竿轮兼容保存校验、求推荐结构化写入等主干设计继续有效。
