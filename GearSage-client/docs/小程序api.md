...
- 错误码：
  ...
  - `500` 未登录 / 服务端异常
- 前端异常处理：
  - 用户停留在详情页期间帖子被后台下架时，点赞接口可能返回 `404 topic not found`；详情页统一提示“帖子已下架”，并将当前页本地交互状态标记为不可点赞/评论/分享
- 分页格式：无
...
- 前端异常处理：
  - 用户停留在详情页期间帖子被后台下架时，发表评论接口可能返回 `404 topic not found`；详情页统一提示“帖子已下架”，关闭评论输入框，并将当前页本地交互状态标记为不可点赞/评论/分享
- 分页格式：无
