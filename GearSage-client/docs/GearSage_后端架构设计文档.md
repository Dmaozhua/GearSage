...
版本：v1.1  
状态：持续维护中 / 已按 2026-03-25 P0 第一版现状同步  
...
- 当前用户资料主链路以 `GET /auth/me` 为主
- `GET /mini/user/info?id=:id` 已实现，但主页面暂未正式消费
- 编辑资料、积分读取已切到独立后台
...
- 评论列表已返回：
  - `likeCount`
  - `isLiked`
- 评论点赞表已采用 `("commentId", "userId")` 唯一约束
- 第一版仍需继续关注“登录切换后再次进入详情页”的最终一致性联调
...
- 第一版主命名已统一为：
  - `uploadToBackend`
  - `generateUploadPath`
- 旧命名 `uploadToCloud / generateCloudPath` 仅保留兼容别名
- 上传接口已支持 `401/403 -> refresh -> 重试一次`
...
- 施工进度与问题回写：`独立后台迁移计划_施工记录版.md`
- 架构与长期设计：`GearSage_后端架构设计文档.md`
- 页面切接口现状：`小程序页面切接口清单.md`
- P0 是否完成：`P0 完成验收清单.md`
- 现有接口与字段契约：`小程序api.md`
...
#### P0
- 评论点赞“登录切换后再次进入详情页”的最终一致性继续联调确认
- 继续做 P0 真机 / 开发者工具回归验收
- 将 P0 文档与仓库实现保持持续同步

#### P1
- COS 接入
- 清理历史 `cloudDB.js` 与未使用云函数适配代码
- 前端继续切标签 / 商城 / 任务接口
- 收装备库迁移方案与接口契约
...
### 16.2 非 P0 模块仍存在云开发残留
风险：

- P0 主链路虽已切完
- 但 `pkgGear`、历史 `cloudDB.js`、`services/cloudApi.js` 等仍保留旧云开发代码
- 后续若误碰这些模块，仍可能出现“局部功能能跑、整体仍未完全脱云”的假象
...
这些逻辑已经进入第一版实现，但仍需继续用真实联调日志验证边缘情况。
