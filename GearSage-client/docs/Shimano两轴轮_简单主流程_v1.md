...
- 零件规格
- 改装/调校向信息
- 论坛沉淀下来的长期使用反馈和深玩家拆解线索

### 深玩家数据当前信任口径

- 对于改装热门型号，`avail / hedgehog_studio` 能抓到深玩家数据是正常预期。
- 对于改装潜力较小、改装热度较低的型号，抓不到深玩家数据也是正常结果，不视为流程失败。
- `tackletour.net` 作为论坛型白名单来源，当前允许全字段抓取并参与深玩家字段研究，但默认仍按 `community_reference` 处理：更适合提供线索、补充经验值和长期使用信息，不默认单站直写高风险结构字段。
...
- 当前阶段，`handle_hole_spec` 允许直接使用白名单站里明确写出的 `M7 / M8` 这类手柄孔规格或锁母规格。
- 当前阶段，`handle_knob_exchange_size` 允许使用白名单站里明确写出的握丸规格、握丸盖型号或轻量握丸类型描述；没有明确规格时保持空值。
