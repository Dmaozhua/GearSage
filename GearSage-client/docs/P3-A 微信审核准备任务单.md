...
### 原则 3：先做可提审主线，按阶段恢复外网联调
在备案与主体条件未满足前，以本地环境推进；当前 ICP 已通过后，恢复 `remote` 环境全链路联调。
...
- 2026-04-13 腾讯云 ICP 已通过，恢复外网联调为第一优先级
...
小程序内落地：
- 2026-05-13 新增 `pages/legal/index?type=` 通用静态制度页，覆盖隐私政策、用户协议、内容发布规范、举报与申诉、账号注销说明、联系我们、关于 GearSage。
- “我的”页直接展示 7 个合规入口；登录页展示用户协议与隐私政策链接；发布编辑、发布模式和发布预览链路展示内容发布规范链接。
...
### 第一优先组（立刻开始）
1. 恢复真正的外网联调（`remote` 环境全链路 smoke）
2. 推进微信小程序主体变更
3. 主体变更成功后立即重提服务类目
4. 做一次“类目-代码一致性”检查
5. 继续推进真实短信接入资料与控制台配置
6. 整理微信提审资料包
...
### 第三优先组（根据结果分支）
8. 如果通过：继续小程序
9. 如果不通过：进入 FinClip 路线

当前进度补充：
- 2026-04-13 已恢复 `remote` 环境第一轮 smoke：
  - `GET https://api.gearsage.club/health` 正常
  - `GET https://api.gearsage.club/health/db` 正常，当前外网库用户为 `gearsage_app`
...
- 当前外网联调首个待修项：
  - 帖子列表 / 详情 / 登录返回的 `avatarUrl / background / images` 仍存在 `http://127.0.0.1:3001/uploads/...` 本地地址
  - 这会导致真机 `remote` 环境下图片资源不可达
  - 当前结论：
    - 外网 API 主链已恢复
    - 远端媒体资源基址配置仍需修正后，才能进入更完整的真机全链路 smoke
- 2026-04-13 已完成远端媒体 URL 第一轮兼容修复：
  - 后端新增统一 media URL 归一化服务
  - 当前接口返回时，会把历史的 `http://127.0.0.1:3001/uploads/...` 自动映射到当前 `UPLOAD_BASE_URL`
  - 已接入：
    - 登录态返回 `auth/login`、`auth/me`
    - 用户资料返回
    - 帖子列表 / 详情 / 草稿
    - 评论列表头像
    - 审核后台帖子看图 / 用户头像
...
- 2026-05-09 微信提审 5 天收口补丁已完成：
  - 首页与登录/关于文案已从“论坛/社区”收口为“装备资料查询、参数对比、装备经验交流”
  - 发布成功提示统一改为“已提交审核”
  - “我的发布”草稿页签中带 `rejectReason` 的驳回内容显示为“未通过”
  ...
  - 本轮本地验证：
    - `npm run build` 通过
    - 本地 `http://127.0.0.1:3025` 验证 `POST /mini/report` -> `GET /admin/reports` -> `POST /admin/reports/:id/handle` 通过
  - 本轮 remote smoke 记录已输出：
    - `docs/微信提审_smoke_记录.md`
  - 微信审核资料包已输出：
    - `docs/微信审核资料包.md`
- 2026-05-09 首页定位二次收口已完成：
  - 底部 tab 从“首页 / 积分 / 我的”调整为“首页 / 装备库 / 我的”
  - 新增主包 `pages/gear-tab/index` 作为装备库 tab 首页，不做 `onShow` 自动跳转
  - 装备库 tab 首页展示装备分类、分类内搜索入口、装备对比入口、最近更新装备
  - 小程序首页首屏调整为“装备资料工具首页 + 装备经验预览”
  - 首页 Hero 文案改为“查装备，问选择，看经验 / 看参数、比技术、找真实使用经验”，点击进入装备库 tab
  - 首页功能入口收口为“装备库 / 装备对比 / 求推荐 / 装备经验”，移除首页积分商城与更多入口
  - 首页内容区改为“最近更新装备 / 最新求推荐 / 装备经验精选”三个传统列表预览模块，不再默认展示瀑布流与排列切换
  - 首页搜索优先进入装备库 tab 并聚焦搜索，不默认伪装为全局搜索，也不默认进入 `type=reels`
  - “装备经验”入口、目标页标题、列表标题已统一为“装备经验”
  - 积分入口已移入“我的”页快捷菜单
- 2026-05-09 发布与审核状态闭环收口已完成：
  - 首页右下角发布入口三项继续保留，不调整首页视觉
  - `publishMode?intent=recommend` 直接进入求推荐表单
  - `publishMode?intent=experience` 直接进入装备经验发布表单
  - `publishMode?intent=question` 直接进入装备问题表单
  - `publishMode` 模式选择页提审期仅展示“求推荐 / 装备经验 / 提问”三类；好物速报、鱼获展示、钓行分享、讨论、晒图闲聊仅保留历史草稿兼容，不作为新发布入口展示
  - 首页“最新求推荐与提问”低压承接 `questionType=recommend/ask`，默认展示 3 条，审核通过后的提问不会脱离首页可见闭环
  - 首页“最新求推荐与提问”的“查看更多”已改为页内展开/收起，不再跳转到无明确承接的搜索页
  - 首页“装备经验精选”的“查看更多”已改为页内展开/收起，不再跳转到静态文章经验页，避免 UGC 装备经验与本地文章资料混用
  - 发布成功提示已确认仍为“已提交审核”
  - 当前审核口径已确认：待审核内容为 `status=1`，公开列表仅展示 `status=2`
  - 待审核内容在前后端均不可评论、不可点赞、不可分享
  - 驳回内容回到草稿并展示“未通过原因”；下架内容通过消息中心通知作者
  - `custom-navbar` 已移除默认右上角“更多”按钮，避免装备库 tab 等提审主路径出现无明确用途入口
  - `pkgContent/detail` 顶部标题已从“帖子详情”改为“装备交流详情”，同页可见状态文案同步收口为“内容”
  - `pkgContent/detail` 顶部导航栏右侧分享图标已移除，避免提审时暴露不必要分享入口
  - `pkgContent/detail` 底部 `share-btn` 已隐藏，详情页提审主路径不再展示分享按钮
  - 首页与装备库 tab 的装备卡片详情跳转已统一传当前装备大类，避免 `item.type` 业务字段覆盖 `reels/rods/lures/line/hook` 后导致详情页子型号与“核心技术”模块异常
  - 装备库 tab 搜索已收口为分类搜索入口，实际搜索统一交给 `pkgGear/pages/list` 既有搜索筛选逻辑
  - 积分页已移除底部操作区；任务数据确认由服务端 `/mini/taskFeat` 返回，当前已在后端返回层与前端展示层同步收口“帖子/邀请”相关文案
  - 积分兑换页提审期仅展示“虚拟权益”，已移除“实体礼品 / 商户券”页签和本地 mock 商品显示
  - 对应验收记录已补入 `docs/微信提审_smoke_记录.md`
