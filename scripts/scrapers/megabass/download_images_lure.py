...
IMAGE_DIR = "/Users/tommy/GearSage/temp_images/megabass_lures"
FINAL_DIR = "/Users/tommy/Pictures/images/megabass_lures"
...
def download_image(url, save_path, final_path):
    if final_path and os.path.exists(final_path):
        print(f"Exists in final, skipping: {final_path}")
        return True
    if os.path.exists(save_path):
        print(f"Exists in temp, skipping: {save_path}")
        return True
...
def process_file(json_filename):
    ...
    def download_item(item):
        ...
        rel_save_path = f"images/megabass_lures/{filename}"
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        results = list(executor.map(download_item, data))
...
if __name__ == "__main__":
    process_file("megabass_lure_normalized.json")
    print("Done!")
