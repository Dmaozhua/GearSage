...
XLSX_PATH = Path("/Users/tommy/GearSage/GearSage-client/pkgGear/data_raw/dstyle_rod_import.xlsx")
...
def read_shared_strings(zf):
    ...
    return ["".join(t.text or "" for t in si.iter(qn("t"))) for si in root.findall(qn("si"))]
...
def cell_text(cell, shared_strings):
    ...
    if cell.get("t") == "inlineStr":
        node = cell.find(qn("is"))
        return "" if node is None else "".join(t.text or "" for t in node.iter(qn("t")))
    v = cell.find(qn("v"))
...
def ensure_fill(styles_root, rgb):
...
def style_variants(styles_root, used_styles, fill_ids):
    ...
    for base in used_styles:
        ...
        for fill_id in fill_ids:
            ...
            variants[(base_idx, fill_id)] = len(cell_xfs.findall(qn("xf"))) - 1
...
def main():
    with zipfile.ZipFile(XLSX_PATH, "r") as zf:
        ...
        rows = sheet_root.find(qn("sheetData")).findall(qn("row"))
        if not rows:
            raise RuntimeError("rod_detail sheet empty")

        header_map = {col_ref(cell.get("r")): cell_text(cell, shared_strings) for cell in rows[0].findall(qn("c"))}
        ...
        for row in rows[1:]:
            ...
            if rod_id != last_rod_id:
                group_idx += 1
                last_rod_id = rod_id
            row_targets.append((row, group_idx))
            for cell in cells:
                used_styles.add(cell.get("s", "0"))

        fill_a = ensure_fill(styles_root, "FFF8F3C8")
        fill_b = ensure_fill(styles_root, "FFE8F1FB")
        variants = style_variants(styles_root, used_styles, [fill_a, fill_b])
        for row, idx in row_targets:
            fill_id = fill_a if idx % 2 == 0 else fill_b
            for cell in row.findall(qn("c")):
                base_style = int(cell.get("s", "0"))
                cell.set("s", str(variants[(base_style, fill_id)]))
        ...
        out = {name: updated.get(name, zf.read(name)) for name in zf.namelist()}
