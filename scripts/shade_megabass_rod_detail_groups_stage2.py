...
XLSX_PATH = Path("/Users/tommy/GearSage/GearSage-client/pkgGear/data_raw/megabass_rod_import.xlsx")
...
def read_shared_strings(zf):
    ...
    for si in root.findall(qn("si")):
        values.append("".join(t.text or "" for t in si.iter(qn("t"))))
    return values
...
def build_style_variants(styles_root, used_styles, fill_ids):
...
def main():
    with zipfile.ZipFile(XLSX_PATH, "r") as zf:
        ...
        variants = build_style_variants(styles_root, used_styles, [fill_a, fill_b])
        ...
        out_bytes = {name: updated_entries.get(name, zf.read(name)) for name in zf.namelist()}
